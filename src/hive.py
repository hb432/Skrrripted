   ...
   for instruction in instructions:
      ...
      if arguments[0] == 'wait':
         ...
         if tick == duration:
            ...
            controller.pitch = 0
      elif arguments[0] == 'position' or arguments[0] == 'velocity' or arguments[0] == 'angular-velocity':
         if tick == duration:
...
class Hive(PythonHivemind):
   ...
   def initialize_hive(self, packet: GameTickPacket) -> None:
      for index in self.drone_indices:
         session['drones'].append({ 'index': index, 'control': PlayerInput(), 'drone': len(session['drones'])})
